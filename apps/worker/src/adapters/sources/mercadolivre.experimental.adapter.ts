...
export class MercadolivreExperimentalAdapter implements SourceAdapter {
  ...
  async extract(): Promise<ExtractedProduct> {
    throw new Error(
      "Mercado Livre adapter is experimental and disabled by default",
    );
  }
  ...
  async createAffiliateLink(_canonicalUrl: URL): Promise<AffiliateLinkResult> {
    return {
      ...
      reason: "Mercado Livre affiliate links require manual insertion",
    };
  }
  ...
  async healthCheck() {
    return {
      healthy: false,
      details: { reason: "experimental_adapter_disabled" },
    };
  }
}
