...
export function registerTelegramRoutes(
  ...
  app.post("/v1/telegram/callback", async (request, reply) => {
    ...
    if (!parsed.success) {
      throw new AppError("VALIDATION_FAILED", "Invalid Telegram callback", false, 400);
    }
    ...
    if (!isAdmin(userId)) {
      ...
      throw new AppError("UNAUTHORIZED", "User not in admin allowlist", false, 403);
    }

    const data = callback_query.data;
    const parts = data.split(":");
    if (parts.length < 3 || parts[0] !== "offer") {
      throw new AppError("VALIDATION_FAILED", "Invalid callback data format", false, 400);
    }
    ...
    logger.info({ action, offerId: offerShortId, userId }, "Telegram callback received");
    ...
    switch (action) {
      case "approve":
        ...
        break;
      case "reject":
        await offerRepository.updateStatus(offerShortId, "rejected", "Discarded by admin");
        break;
      case "revalidate":
        logger.info({ offerId: offerShortId }, "Revalidation requested via callback");
        break;
      case "edit":
      ...
      case "schedule":
        ...
        break;
      default:
        throw new AppError("VALIDATION_FAILED", `Unknown action: ${action}`, false, 400);
    }
